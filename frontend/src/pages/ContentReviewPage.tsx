...
export default function ContentReviewPage() {
  ...
  async function handleReview() {
    ...
    const res = await contentApi.review(siteId, content, kwList, token);
    setRunning(false);
  ...
  return (
    <div className="fade-in max-w-4xl p-6 lg:p-8">
      ...
      {/* Header */}
      <div className="mb-6">
        <span className="text-sage/40 text-[10px] tracking-wider uppercase">Content SEO Review</span>
        <h1 className="font-heading text-cream mt-0.5 text-xl font-semibold">Content Analysis</h1>
        <p className="text-sage/60 mt-0.5 text-sm">
      ...
      <div className="mb-6 space-y-4">
        {/* Target keywords */}
        <div>
          <label className="text-sage/80 mb-1.5 block text-xs font-medium tracking-wide">Target Keywords</label>
          <input
            value={targetKeywords}
            onChange={(e) => setTargetKeywords(e.target.value)}
            placeholder="content marketing, SEO strategy, blog writing"
            className="text-cream placeholder:text-sage/40 focus:border-clay/60 focus:ring-clay/20 w-full rounded-lg border border-white/10 bg-white/[0.04] px-3.5 py-2.5 text-sm transition-colors hover:border-white/20 focus:ring-1 focus:outline-none"
          />
          <p className="text-sage/40 mt-1 text-[10px]">Comma-separated</p>
        </div>

        {/* Content */}
        <div>
          <label className="text-sage/80 mb-1.5 block text-xs font-medium tracking-wide">Page Content</label>
          <textarea
            value={content}
            onChange={(e) => setContent(e.target.value)}
            placeholder="Paste the full text content of the page you want to analyze…"
            rows={8}
            className="text-cream placeholder:text-sage/40 focus:border-clay/60 focus:ring-clay/20 w-full resize-y rounded-lg border border-white/10 bg-white/[0.04] px-3.5 py-2.5 text-sm transition-colors hover:border-white/20 focus:ring-1 focus:outline-none"
          />
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-5">
            <div className="mb-3 flex items-center justify-between">
              <p className="font-heading text-cream text-sm font-semibold">Overall Assessment</p>
              <span
                className={`rounded-full border px-2 py-0.5 text-[10px] font-medium ${READABILITY_STYLES[result.estimatedReadability] ?? 'text-sage/40 border-white/[0.06] bg-white/[0.04]'}`}
              >
                Readability: {result.estimatedReadability}
              </span>
            </div>
            <p className="text-sage/70 text-sm leading-relaxed">{result.overallAssessment}</p>
          </div>

          {/* Suggestions */}
          <div className="overflow-hidden rounded-xl border border-white/[0.06] bg-white/[0.02]">
            <div className="border-b border-white/[0.06] px-5 py-3">
              <p className="font-heading text-cream text-sm font-semibold">Suggestions ({result.suggestions.length})</p>
            </div>
              ...
              {result.suggestions.map((s, i) => (
                <div key={i} className="px-5 py-4">
                  <div className="mb-1.5 flex items-start gap-2">
                    ...
                    <p className="text-cream text-sm font-medium">{s.issue}</p>
                  </div>
                  <div className="flex items-start gap-2">
                    ...
                    <p className="text-sage/70 text-sm">{s.recommendation}</p>
                  </div>
                </div>
              ))}
            </div>
